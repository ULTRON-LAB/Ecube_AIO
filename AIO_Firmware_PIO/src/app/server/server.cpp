...
struct ServerAppRunData
{
    ...
    boolean wifi_connected;               // 标志是否已成功连接 Wi-Fi
};
...
{
    // 首页
    server.on("/", HTTP_GET, sys_setting);
                           ...
                           const ImuAction *action)
{
    ...
    if (0 == run_data->web_start && 0 == run_data->req_sent)
    {
        ...
        sys->send_to(SERVER_APP_NAME, CTRL_NAME, APP_MESSAGE_WIFI_CONN, NULL, NULL);
                                  ...
                                  void *ext_info)
{
    switch (type)
    {
    case APP_MESSAGE_WIFI_CONN: //2024.12.3新增
    {
        Serial.print(F("APP_MESSAGE_WIFI_CONN connected\n"));
        run_data->wifi_connected = true; // 标记 Wi-Fi 已连接
    }
    break;
