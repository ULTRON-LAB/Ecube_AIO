...
#define SYS_SETTING "<form method=\"GET\" action=\"saveSysConf\">"                                                                                                                                                                                                      \
                    ...
                    "<label class=\"input\"><span>開機自啟的APP名字</span><select name=\"auto_start_app\">"   \
                                    "<option value=\"\"            %s>None</option>"  \
                                    "<option value=\"Weather\"     %s>Weather</option>"  \
                                    "<option value=\"Tomato\"      %s>Tomato</option>"    \
                                    "<option value=\"Picture\"     %s>Picture</option>"  \
                                    "<option value=\"Media\"       %s>Media</option>"      \
                                    "<option value=\"WebServer\"   %s>WebServer</option>"  \
                                    "<option value=\"Idea\"        %s>Idea</option>"    \
                                    "<option value=\"Settings\"    disabled>Settings(暫不支援)</option>"    \
                                    "<option value=\"2048\"        disabled>2048(暫不支援)</option>"    \
                                    "<option value=\"Anniversary\" %s>Anniversary</option>"  \
                                    "<option value=\"PC Resource\" %s>PC Resource</option>"  \
                                    "<option value=\"LH&LXW\" disabled>LH&LXW(暫不支援)</option>"  \
                                "</select></label>"                                                                                                                      \
...
void sys_setting()
{
    ...
    // 確認選擇的APP
    const char* auto_start_list[] = {
        "","Weather", "Tomato", "Picture", "Media", "Webserver", "Idea", 
        "Anniversary", "PC Resource"
    };

    for (int i = 0; i < 9; ++i)
    {
        if (strcmp(auto_start_app, auto_start_list[i]) == 0)
        {
...
void File_Upload()
{
    ...
    webpage += "<h3>Select File to Upload</h3>"
           ...
           "<input class='buttons' style='width:40%' type='file' name='fupload' id='fupload' value='' accept='.jpg,.jpeg,.mjpeg'><br>"
           "<h3>提示:僅可上傳240*240尺寸的JPG(圖片)或MJPEG(影片)檔，請參照說明書使用隨附的轉檔軟體</h3>"
           "<div style='display: flex; flex-direction: column; align-items: center;'>"
           "    <h4 id='previewTitle' style='display: none;'>靜態預覽</h4>"
           "    <div style='text-align: center;'>"
           "        <img id='imagePreview' style='width:240px; height:240px; display:none;' /><br>"
           "        <img id='videoPreview' style='width:240px; height:240px; display:none;' /><br>"
           "    </div>"
           "</div>"
           "<button class='buttons' style='width:10%' type='submit'>Upload File</button><br>"
           "<a href='/'>[Back]</a><br><br>"
           "<script>"
           "document.getElementById('fupload').onchange = function(event) {"
