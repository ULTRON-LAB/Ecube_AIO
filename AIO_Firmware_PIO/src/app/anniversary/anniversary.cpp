...
#define TIME_API "https://api.timezonedb.com/v2.1/get-time-zone?key=XPU1UNS03GGS&format=json&by=zone&zone=Asia/Taipei"
...
static void read_config(AN_Config *cfg)
{
    ...
    if (size == 0)
    {
        ...
        cfg->event_name[0] = "生日還有";
        cfg->target_date[0].tm_year = 0; // 设置为零则每年重复
        cfg->target_date[0].tm_mon = 7;
        cfg->target_date[0].tm_mday = 30;
        cfg->event_name[1] = "畢業還有";
        cfg->target_date[1].tm_year = 2027;
...
static long long get_timestamp(String url)
{
    ...
    if (httpCode > 0)
    {
        if (httpCode == HTTP_CODE_OK)
        {
            ...
            // 查找 "timestamp" 的位置
            int time_index = payload.indexOf("\"timestamp\":") + 12;
            
            // 確認 "timestamp" 存在於回傳的 JSON 中
            if (time_index != 11)
            {
                // 查找字段結尾的逗號或右大括號的位置
                int time_end_index = payload.indexOf(",", time_index);
                if (time_end_index == -1) {
                    time_end_index = payload.indexOf("}", time_index);
                }
                ...
                if (time_end_index != -1)
                {
                    ...
                    run_data->preNetTimestamp = atoll(time.c_str()) * 1000 + run_data->errorNetTimestamp + TIMEZERO_OFFSIZE;  //時區補償暫設為0
                    run_data->preLocalTimestamp = GET_SYS_MILLIS();
                }
                else
                {
                    Serial.println("[ERROR] JSON parsing error: Unable to find the end of timestamp value");
                }
            }
            else
            {
                Serial.println("[ERROR] JSON parsing error: timestamp field not found");
            }
        }
    }
    else
